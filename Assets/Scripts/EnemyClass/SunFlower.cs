...
public class SunFlower : Enemy
{
    ...
    public void Heal(float range)
    {
        ...
        foreach (Collider hit in hits)
        {
            if (hit.gameObject.CompareTag("Enemy"))
            {
                ...
                if (eb == null)
                {
                    hit.gameObject.AddComponent<EnemyBuff_20601>();
                    eb = hit.gameObject.GetComponent<EnemyBuff_20601>();
                    eb.enabled = true;
    ...
    {
        // 检查是否为友方单位
        Debug.Log("collider is exiting");
        if (other.gameObject.CompareTag("Enemy"))
        {
            // 移除组件
            
            EnemyBuff_20601 buff = other.gameObject.GetComponent<EnemyBuff_20601>();
            if (buff != null)
            {
                Destroy(buff);
                Debug.Log("Buff is already been destroyed");
            }
        }
    }
